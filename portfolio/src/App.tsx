...
import { extra, how, tech } from './tech'

const EXPO = [0.16, 1, 0.3, 1] as const
const SWIFT = [0.7, 0, 0.2, 1] as const
let lenis: Lenis | null = null

type Proj = Meta & PText & { tech: string[] }
const I18n = createContext<{ lang: Lang; t: Content; setLang: (l: Lang) => void; projects: Proj[]; touch: boolean }>(null as never)
    ...
    <span ref={ref}>
      {lines.map((l, i) => (
        <span key={i} className="block overflow-hidden pb-[.16em] -mb-[.16em]">
          <motion.span className="block" initial={{ y: '112%' }} animate={seen && start ? { y: 0 } : { y: '112%' }}
...
function Fade({ children, delay = 0, className = '' }: { children: ReactNode; delay?: number; className?: string }) {
  return (
    <motion.div className={className} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, margin: '0px 0px -12% 0px' }} transition={{ duration: 0.9, ease: EXPO, delay }}>{children}</motion.div>
  )
}
...
function Magnetic({ children, strength = 0.35 }: { children: ReactNode; strength?: number }) {
  ...
  if (touch) return <span className="inline-block">{children}</span>
...
/** Ảnh chụp app: bản 380px cho điện thoại, 600px cho máy tính */
function Shot({ src, className = '', sizes = '(max-width: 768px) 60vw, 260px', alt = '', eager = false }: { src: string; className?: string; sizes?: string; alt?: string; eager?: boolean }) {
  const sm = src.replace('.jpg', '-sm.jpg')
  return <img src={src} srcSet={`${sm} 380w, ${src} 600w`} sizes={sizes} alt={alt} loading={eager ? 'eager' : 'lazy'} decoding="async" className={className} draggable={false} />
}
...
const go = (id: string) => (e?: { preventDefault: () => void }) => {
  ...
  if (el) lenis ? lenis.scrollTo(el, { duration: 1.5 }) : el.scrollIntoView({ behavior: 'smooth' })
}
...
function Preloader({ onDone }: { onDone: () => void }) {
  ...
  return (
    <motion.div className="fixed inset-0 z-[300] bg-[var(--ink)] text-[var(--paper)]" exit={{ y: '-125%' }} transition={{ duration: 1, ease: SWIFT }}>
...
function Menu() {
  ...
  useEffect(() => { lockBody(open); return () => lockBody(false) }, [open])
  ...
  const jump = (h: string) => () => { setOpen(false); setTimeout(() => go(h)(), 420) }
  return (
    <>
      <motion.div className="fixed right-[clamp(16px,3vw,40px)] z-[120]" style={{ top: 'calc(env(safe-area-inset-top, 0px) + 22px)' }}
        ...
        <Magnetic strength={0.4}>
          <button onClick={() => setOpen(!open)} aria-label={t.nav.menu} aria-expanded={open}
            className={`grid h-[68px] w-[68px] place-items-center rounded-full transition-colors duration-500 active:scale-95 max-sm:h-[58px] max-sm:w-[58px] ${open ? 'bg-[var(--leaf)]' : 'bg-[var(--ink)]'}`}>
      ...
      <AnimatePresence>
        {open && (
          <>
            ...
            <motion.aside className="fixed inset-y-0 right-0 z-[110] w-[min(580px,100vw)] bg-[var(--ink)] text-[var(--paper)]"
              ...
              <div className="flex h-full flex-col justify-between px-[clamp(28px,5vw,90px)] pb-12 pt-[18vh]" style={{ paddingBottom: 'calc(48px + env(safe-area-inset-bottom, 0px))' }}>
                <div>
                  ...
                  <ul className="mt-8">
                    {links.map(([h, l], i) => (
                      <motion.li key={h} initial={{ x: 90, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 60, opacity: 0 }} transition={{ duration: 0.85, ease: EXPO, delay: 0.12 + i * 0.06 }}>
                        <button onClick={jump(h)} className="group flex items-center gap-4 py-2 text-[clamp(36px,4.2vw,58px)] leading-[1.15] tracking-[-0.03em] active:opacity-60">
...
const NAME_ROW = [0, 1, 2, 3].map((k) => <span key={k} className="pr-[.35em]">Vương Hỷ Khang <span className="serif">—</span></span>)

/** Máy tính: tốc độ đổi theo chiều cuộn. Điện thoại: keyframe CSS thuần, chạy trên compositor. */
function NameMarquee() {
  ...
  const cls = 'display flex w-max text-[clamp(80px,15.5vw,250px)] leading-[1.2]'
  return (
    <div className="overflow-hidden whitespace-nowrap" aria-hidden>
      {touch ? <div className={`${cls} marquee-css`}>{NAME_ROW}</div> : <motion.div style={{ x }} className={cls}>{NAME_ROW}</motion.div>}
    </div>
  )
}
...
function Portrait({ ready }: { ready: boolean }) {
  const { lang } = useT()
  return (
    <div className="w-full md:w-[min(400px,100%)]">
      <motion.div className="relative aspect-[4/5] overflow-hidden bg-[#DAD8CF]"
        initial={{ clipPath: 'inset(100% 0% 0% 0%)' }} animate={ready ? { clipPath: 'inset(0% 0% 0% 0%)' } : {}} transition={{ duration: 1.25, ease: SWIFT, delay: 0.3 }}>
        <motion.img src="assets/portrait.jpg" srcSet="assets/portrait-sm.jpg 640w, assets/portrait.jpg 1100w" sizes="(max-width: 768px) 100vw, 400px"
          alt="Vương Hỷ Khang" fetchPriority="high" decoding="async" className="absolute inset-0 h-full w-full object-cover"
          initial={{ scale: 1.18 }} animate={ready ? { scale: 1 } : {}} transition={{ duration: 1.6, ease: EXPO, delay: 0.3 }} />
      </motion.div>
      <motion.div initial={{ opacity: 0 }} animate={ready ? { opacity: 1 } : {}} transition={{ duration: 1, delay: 1.3 }} className="mono mt-4 flex justify-between text-[var(--grey)]">
        <span>{extra[lang].caption}</span><span>Flutter · iOS</span>
      </motion.div>
    </div>
  )
}
...
function Hero({ ready }: { ready: boolean }) {
  ...
  return (
    <section id="top" className="relative flex min-h-[100svh] flex-col overflow-hidden">
      ...
      <motion.div style={touch ? undefined : { y }} className="wrap grid flex-1 items-center gap-x-10 gap-y-12 pb-8 pt-32 md:grid-cols-12">
        ...
        <div className="md:col-span-5 md:flex md:justify-end"><Portrait ready={ready} /></div>
      </motion.div>
...
function Intro() {
  const { t, touch } = useT()
  const words = useMemo(() => t.intro.text.split(/(?<=[\s，。：；、])/u).flatMap((w) => (w.length > 8 && !/\s/.test(w) ? w.match(/.{1,4}/gu)! : [w])), [t])
  const ref = useRef<HTMLParagraphElement>(null)
  const { scrollYProgress } = useScroll({ target: ref, offset: ['start 85%', 'end 45%'] })
  useMotionValueEvent(scrollYProgress, 'change', (v) => { if (!touch) ref.current?.style.setProperty('--p', String(v * 1.12)) })
  return (
    <section className="wrap grid gap-10 py-[clamp(90px,14vw,200px)] md:grid-cols-12">
      ...
      {touch ? (
        <Fade className="text-[clamp(24px,2.9vw,42px)] leading-[1.34] tracking-[-0.02em] md:col-span-9"><p>{t.intro.text}</p></Fade>
      ) : (
...
function WorkIndex({ onOpen }: { onOpen: (id: string) => void }) {
  ...
  return (
    <section id="work" className="relative pb-[clamp(70px,9vw,130px)]" onMouseMove={touch ? undefined : (e) => { mx.set(e.clientX); my.set(e.clientY) }}>
      <div className="wrap">
          ...
          {projects.map((p, i) => (
            <li key={p.id} className="border-b border-[var(--hair)]">
              <button onClick={() => onOpen(p.id)} onMouseEnter={() => { setActive(i); setLast(i) }} onFocus={() => { setActive(i); setLast(i) }}
                className="group grid w-full grid-cols-[1fr_auto] items-center gap-5 py-[clamp(22px,3.3vw,52px)] transition-opacity duration-500 active:opacity-60" style={{ opacity: on && active !== i ? 0.3 : 1 }}>
                <span className="display flex items-center gap-[.35em] text-[clamp(30px,6vw,96px)] transition-transform duration-700 ease-[cubic-bezier(.16,1,.3,1)] group-hover:translate-x-[.22em]">
                  {p.icon && <img src={p.icon} alt="" className="hidden h-[.62em] w-[.62em] rounded-[22%] sm:block" />}{p.name}
                </span>
                <span className="flex items-center gap-4 text-right transition-transform duration-700 ease-[cubic-bezier(.16,1,.3,1)] group-hover:-translate-x-[1.2em]">
                  <span><span className="block text-[clamp(14px,1.2vw,18px)]">{p.kind}</span><span className="mono text-[var(--grey)]">{p.year}</span></span>
                  <span className="grid h-[72px] w-[56px] place-items-center overflow-hidden rounded-[10px] md:hidden" style={{ background: p.panel }}>
                    {p.shots ? <Shot src={p.shots[0]} sizes="56px" className="h-[86%] w-auto rounded-[5px]" /> : <span className="mono text-[10px]" style={{ color: p.ink }}>{p.year}</span>}
                  </span>
                </span>
              </button>
            </li>
            ...
            <div className="h-full w-full transition-transform duration-[650ms] ease-[cubic-bezier(.7,0,.2,1)]" style={{ transform: `translate3d(0,-${last * 100}%,0)` }}>
              {projects.map((p) => (
                <div key={p.id} className="grid h-full w-full place-items-center" style={{ background: p.panel, color: p.ink }}>
                  {p.shots ? <Shot src={p.shots[0]} sizes="200px" className="h-[84%] w-auto rounded-[22px]" /> : <span className="display px-8 text-center text-[42px]">{p.name}</span>}
                </div>
              ))}
            </div>
          </motion.div>
...
/* ---------------------------------------------------------------- */
/* reel (máy tính) / thẻ xếp chồng (điện thoại)                       */
/* ---------------------------------------------------------------- */
...
function Reel() {
  ...
  return (
    <div ref={ref} className="space-y-[clamp(12px,1.6vw,24px)] overflow-hidden pb-[clamp(90px,12vw,170px)]" aria-hidden>
      {REEL.map((r, k) => (
        <motion.div key={k} style={{ x: k ? b : a }} className="flex w-max gap-[clamp(12px,1.6vw,24px)]">
          {r.map((tile) => (
            <div key={tile.src} className="relative aspect-[4/3] w-[clamp(240px,29vw,460px)] overflow-hidden" style={{ background: tile.bg }}>
              <Shot src={tile.src} sizes="180px" className="absolute left-1/2 top-[13%] w-[40%] -translate-x-1/2 rounded-[clamp(12px,1.3vw,20px)]" />
            </div>
          ))}
        </motion.div>
      ))}
...
/** Điện thoại: mỗi app một thẻ dính ở đầu màn hình, thẻ sau đè lên thẻ trước — chỉ dùng position: sticky */
function StackCards({ onOpen }: { onOpen: (id: string) => void }) {
  const { projects, t } = useT()
  const items = projects.filter((p) => p.shots)
  return (
    <div className="wrap pb-[14vh]">
      {items.map((p, i) => (
        <div key={p.id} className="sticky" style={{ top: `calc(env(safe-area-inset-top, 0px) + ${64 + i * 12}px)`, marginBottom: i === items.length - 1 ? 0 : '18vh' }}>
          <button onClick={() => onOpen(p.id)} className="block w-full overflow-hidden rounded-[22px] text-left shadow-[0_-8px_30px_-16px_rgba(0,0,0,.25)]" style={{ background: p.panel, color: p.ink }}>
            <div className="flex items-center justify-between px-5 pt-5">
              <div className="flex items-center gap-3"><img src={p.icon} alt="" className="h-11 w-11 rounded-[22%]" /><div><p className="text-[19px] leading-tight">{p.name}</p><p className="mono mt-0.5 opacity-60">{p.kind} · {p.year}</p></div></div>
              <span className="mono opacity-60">{t.work.view} →</span>
            </div>
            <p className="px-5 pt-4 text-[15.5px] leading-[1.5] opacity-85">{p.lead}</p>
            <div className="mt-5 flex justify-center px-5">
              <Shot src={p.shots![0]} sizes="55vw" className="w-[55%] max-w-[230px] translate-y-3 rounded-t-[22px]" />
            </div>
          </button>
        </div>
      ))}
...
function FeatureBlock({ i, title, text, onActive, compact }: { i: number; title: string; text: string; onActive: (i: number) => void; compact: boolean }) {
  const ref = useRef<HTMLDivElement>(null)
  const inView = useInView(ref, { margin: compact ? '-60% 0px -25% 0px' : '-48% 0px -48% 0px' })
  useEffect(() => { if (inView) onActive(i) }, [inView, i, onActive])
  return (
    <div ref={ref} className={compact ? 'flex min-h-[38svh] flex-col justify-center py-6' : 'flex min-h-[62vh] flex-col justify-center'}>
      <p className="mono opacity-60">{String(i + 1).padStart(2, '0')}</p>
      <h4 className="mt-3 text-[clamp(24px,2.8vw,40px)] leading-[1.14] tracking-[-0.025em]">{title}</h4>
      <p className="mt-3 max-w-[40ch] text-[17px] leading-[1.6] opacity-80">{text}</p>
    </div>
  )
}
...
function Spotlight({ p, onOpen }: { p: Proj; onOpen: (id: string) => void }) {
  ...
  const phone = (h: string) => (
    <div className={`relative aspect-[506/1100] ${h} overflow-hidden rounded-[clamp(22px,4vw,38px)] bg-black/10`} style={{ boxShadow: '0 40px 80px -40px rgba(0,0,0,.5)' }}>
      {shots.map((s, i) => (
        <motion.div key={s} className="absolute inset-0" initial={false} animate={{ opacity: a === i ? 1 : 0 }} transition={{ duration: 0.6, ease: [0.32, 0.08, 0.24, 1] }}>
          <Shot src={s} sizes="(max-width: 768px) 40vw, 320px" className="h-full w-full object-cover" eager={i === 0} />
        </motion.div>
      ))}
    </div>
  )
  return (
    <section style={{ background: p.panel, color: p.ink }}>
      <div className="wrap py-[clamp(60px,9vw,130px)]">
        <div className="flex flex-wrap items-end justify-between gap-8 border-b pb-10" style={{ borderColor: `${p.ink}2E` }}>
          <div className="flex items-center gap-5">
            <img src={p.icon} alt="" className="h-[92px] w-[92px] rounded-[22%] max-sm:h-[64px] max-sm:w-[64px]" />
            <div><p className="mono opacity-60">{p.kind} · {p.year}</p><h3 className="display mt-2 text-[clamp(32px,5vw,72px)]"><Lines lines={[p.name]} /></h3></div>
          </div>
          <Fade className="max-w-[44ch] text-[18px] leading-[1.5]">{p.lead}</Fade>
        </div>

        {touch ? (
          <div>
            <div className="sticky top-0 z-10 flex h-[48svh] items-end justify-center pb-3 pt-4" style={{ background: p.panel }}>{phone('h-[calc(48svh-36px)]')}</div>
            <div className="pt-4">{feats.map((f, i) => <FeatureBlock key={f.title} i={i} title={f.title} text={f.text} onActive={setA} compact />)}</div>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-x-16">
            <div><div className="sticky top-0 grid h-screen place-items-center">{phone('h-[min(74vh,680px)]')}</div></div>
            <div className="min-w-0 py-[19vh]">{feats.map((f, i) => <FeatureBlock key={f.title} i={i} title={f.title} text={f.text} onActive={setA} compact={false} />)}</div>
          </div>
        )}
        <div className="flex flex-wrap gap-x-8 gap-y-3 pt-8 text-[17px] md:-mt-[12vh] md:pl-[calc(50%+32px)]">
...
function HowIBuild() {
  ...
  const h = how[lang]
  return (
    <section className="cv-auto border-t border-[var(--hair)]">
      <div className="wrap py-[clamp(90px,12vw,170px)]">
        <div className="grid gap-x-10 gap-y-8 md:grid-cols-12">
          <p className="mono text-[var(--grey)] md:col-span-3">{h.label}</p>
          <div className="md:col-span-9">
            <h2 className="display max-w-[22ch] text-[clamp(30px,4.2vw,60px)]"><Lines lines={[<Rich key="t" s={h.title} />]} /></h2>
            <Fade className="mt-5 max-w-[50ch] text-[var(--grey)]">{h.intro}</Fade>
          </div>
        </div>
        <ol className="mt-[clamp(40px,6vw,80px)] grid border-t border-[var(--hair)] md:grid-cols-2 md:gap-x-10">
          {h.notes.map((n, i) => (
            <motion.li key={n} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '0px 0px -10% 0px' }} transition={{ duration: 0.8, ease: EXPO, delay: (i % 2) * 0.06 }}
              className="grid grid-cols-[44px_1fr] gap-4 border-b border-[var(--hair)] py-5">
              <span className="mono pt-1 text-[var(--grey)]">{String(i + 1).padStart(2, '0')}</span>
              <span className="text-[16.5px] leading-[1.55]">{n}</span>
            </motion.li>
          ))}
        </ol>
        <div className="mt-[clamp(40px,6vw,80px)] grid gap-x-10 gap-y-6 md:grid-cols-12">
          <p className="mono text-[var(--grey)] md:col-span-3">{h.toolsLabel}</p>
          <dl className="border-t border-[var(--hair)] md:col-span-9">
            {h.tools.map(([k, v]) => (
              <div key={k} className="grid gap-x-6 gap-y-1 border-b border-[var(--hair)] py-4 md:grid-cols-[140px_1fr]"><dt className="mono text-[var(--grey)]">{k}</dt><dd className="m-0 text-[16.5px]">{v}</dd></div>
            ))}
          </dl>
        </div>
      </div>
...
function Archive() {
  const { t } = useT()
  return (
    <section className="cv-auto wrap py-[clamp(90px,12vw,170px)]">
...
function About() {
  const { t } = useT()
  return (
    <section id="about" className="cv-auto border-t border-[var(--hair)]">
      <div className="wrap py-[clamp(90px,12vw,170px)]">
...
function Footer() {
  ...
  const pill = 'group block rounded-full border border-[var(--hair-dark)] px-7 py-4 transition-colors duration-500 hover:border-[var(--paper)] active:bg-white/10'
  return (
    <footer id="contact" ref={ref} className="relative overflow-hidden bg-[var(--ink)] text-[var(--paper)]">
      <motion.div aria-hidden className="absolute inset-x-[-10%] -top-px h-[11vw] origin-top bg-[var(--paper)]" style={{ scaleY: cap, borderRadius: CAP_DOWN }} />
      <motion.div style={touch ? undefined : { y }} className="wrap pb-10 pt-[clamp(150px,18vw,260px)]">
        ...
        <div className="relative mt-[clamp(40px,6vw,90px)] border-t border-[var(--hair-dark)]">
          <div className="absolute right-[8%] top-0 -translate-y-1/2 max-sm:right-4">
            <Magnetic>
              <a href={`mailto:${EMAIL}`} className="group relative grid h-[172px] w-[172px] place-items-center overflow-hidden rounded-full bg-[var(--leaf)] text-[#F3F2EC] active:scale-95 max-sm:h-[132px] max-sm:w-[132px]">
        ...
        <div className="flex flex-wrap gap-3 pt-[clamp(110px,10vw,140px)]">
        ...
        <div className="mt-[clamp(60px,8vw,110px)] flex flex-wrap justify-between gap-6" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
...
function CaseStudy({ p, onClose, onNext }: { p: Proj; onClose: () => void; onNext: () => void }) {
  const { t, lang, projects } = useT()
  const next = projects[(projects.findIndex((x) => x.id === p.id) + 1) % projects.length]
  const scroller = useRef<HTMLDivElement>(null)
  useEffect(() => {
    const k = (e: KeyboardEvent) => e.key === 'Escape' && onClose()
    window.addEventListener('keydown', k); lockBody(true)
    return () => { window.removeEventListener('keydown', k); lockBody(false) }
  }, [onClose])
  useEffect(() => { scroller.current?.scrollTo(0, 0) }, [p.id])
  return (
    <motion.div role="dialog" aria-modal="true" aria-label={p.name} className="fixed inset-0 z-[200]"
      ...
      <div ref={scroller} data-lenis-prevent className="no-scrollbar h-full overflow-y-auto overscroll-contain bg-[var(--paper)]">
        <div className="sticky top-0 z-10 border-b border-[var(--hair)] bg-[var(--paper)]" style={{ paddingTop: 'env(safe-area-inset-top, 0px)' }}>
          <div className="wrap flex justify-between py-5"><span>{p.name}</span><button onClick={onClose} className="group active:opacity-60"><Roll>{t.work.close} ✕</Roll></button></div>
        </div>
        <div className="wrap pb-20 pt-[clamp(40px,7vw,110px)]">
          <p className="mono text-[var(--grey)]">{p.kind} · {p.year}</p>
          <h2 className="display mt-5 text-[clamp(40px,9.5vw,150px)]"><Lines delay={0.5} lines={[p.name]} /></h2>
          <div className="mt-[clamp(40px,6vw,90px)] grid gap-x-10 gap-y-12 md:grid-cols-12">
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.75, duration: 1, ease: EXPO }} className="md:col-span-7 md:col-start-6">
              ...
              <p className="mono mt-12 border-b border-[var(--hair)] pb-4 text-[var(--grey)]">{extra[lang].techLabel}</p>
              <ul className="mt-2">
                {p.tech.map((x) => <li key={x} className="grid grid-cols-[18px_1fr] gap-3 border-b border-[var(--hair)] py-3.5 text-[15.5px] leading-[1.55] text-[#3c403c]"><span className="pt-[9px]"><span className="block h-1.5 w-1.5 rounded-full bg-[var(--leaf)]" /></span>{x}</li>)}
              </ul>
            </motion.div>
          </div>
        </div>
        {p.shots && (
          <div style={{ background: p.panel }} className="py-[clamp(50px,7vw,110px)]">
            <div className="no-scrollbar wrap flex snap-x snap-mandatory gap-[clamp(16px,2.4vw,36px)] overflow-x-auto md:justify-center">
              {p.shots.map((s, i) => (
                <motion.div key={s} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, root: scroller }} transition={{ duration: 1, ease: EXPO, delay: i * 0.09 }} className="w-[min(60vw,270px)] shrink-0 snap-center">
                  <Shot src={s} alt={`${p.name} ${i + 1}`} sizes="(max-width: 768px) 60vw, 270px" className="w-full rounded-[28px]" />
                </motion.div>
              ))}
            </div>
          </div>
        )}
        <button onClick={onNext} className="group block w-full border-t border-[var(--hair)] py-[clamp(60px,8vw,120px)] text-center active:opacity-60">
...
export default function App() {
  ...
  const ctx = useMemo(() => {
    ...
    return { lang, t, setLang, touch, projects: meta.map((m) => ({ ...m, ...t.projects[m.id], tech: tech[lang][m.id] ?? [] })) }
  }, [lang, setLang, touch])

  /* Cuộn mượt chỉ trên chuột/trackpad — trên màn cảm ứng để trình duyệt tự cuộn (như Dennis, Olivier, Cuberto) */
  useEffect(() => {
    if (reduce || touch) return
    lenis = new Lenis({ lerp: 0.12, autoRaf: true })
    return () => { lenis?.destroy(); lenis = null }
  }, [reduce, touch])
  useEffect(() => { if (loading) { lockBody(true); window.scrollTo(0, 0) } else lockBody(false) }, [loading])
  ...
  return (
    <I18n.Provider value={ctx}>
      ...
      <main key={lang}>
        ...
        {narrow ? <StackCards onOpen={setOpenId} /> : <Reel />}
